    ...
    >
      <template slot-scope="{ item, row }">
        <span
          ...
          @click="openDialog(row)"
        >{{ row[item.name] }}</span>
        <span v-else-if="item.name ==='DEAL_ORGAN'">{{
          row[item.name] | organNameFormat
        }}</span>
        <span v-else-if="item.name === 'DAT_POST'">
...
import { WarningReportSupple } from '@/api'
const { reportSupple } = WarningReportSupple
export default {
  name: '',
  ...
  methods: {
    ...
    queryList(currentPage) {
      ...
      // 查询请求
      reportSupple(msg).then((response) => {
