      ...
      <template slot="tableColumn">
        ...
        >
          <div slot-scope="{ row }">
            ...
            <span
              v-else-if="
                item.name === 'warn_date' || item.name === 'expire_date'
              "
            >
...
import { WarningPapersWarn } from '@/api'
const { papersWarn, accInfo } = WarningPapersWarn
export default {
  ...
  methods: {
    ...
    handleExport() {
      ...
      exportXlsx(tHeader, data, '证件到期预警信息')
    },
    // 选择单行
    currentChange(val) {
      this.currentRow = val
