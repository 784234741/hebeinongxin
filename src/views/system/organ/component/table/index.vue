    ...
    >
      <template slot="tableColumn">
        ...
        >
          <div slot-scope="{ row }">
            <span
              v-if="item.name === 'organ_no' || item.name === 'parent_organ'"
            >{{ row[item.name] | organNameFormat }}</span>
            <span v-else-if="item.name === 'organ_type'">{{
              row[item.name] | organType
            }}</span>
            <span v-else-if="item.name === 'status'">{{
              row[item.name] | status
            }}</span>
            ...
            <span v-else-if="item.name === 'organ_level'">{{
              (row[item.name] + '') | organLevel
            }}</span>
            ...
            parameterList: [
              {
                ...
                organ_name: formData1.organ_name,
                parent_organ: formData1.parent_organ,
                organ_type: formData1.organ_type,
                organ_level: formData1.organ_level,
                status: formData1.status,
                report_way: formData1.report_way,
                bank_code: formData1.bank_code,
                city_name: formData1.city_name,
                area_name: formData1.organ_no,
                organ_phone: formData1.organ_phone,
                address: formData1.address,
                longitude: formData1.longitude,
                latitude: formData1.latitude,
