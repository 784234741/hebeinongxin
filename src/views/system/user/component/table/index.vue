...
export default {
  ...
  created() {
    this.listLoading = this.loading
    this.userLevel = this.$store.getters.roleNo
  },
  methods: {
    // 表格选择多行
     ...
     * btn - 新增*/
    handleAdd() {
      this.rolelistCopy = JSON.parse(JSON.stringify(this.rolelist))
      this.disabledLevel(this.userLevel, this.rolelistCopy)
      ...
      role_no.options = this.rolelistCopy
    ...
    disabledLevel(num, datas) {
      console.log(num)
      let tellerData = dictionaryFieds('USER_LEVEL')
      const { children } = datas[0]
      if (num === '11') { // 省级
        this.dialog.formData.tellerlvl = '2'
        this.dialog.config.tellerlvl.options = tellerData.filter(item => item.value !== '1')
        const index = children.findIndex(item => item.id === '11')
        if (index >= 0) {
          children.splice(index, 1)
        }
      } else if (num === '22') { // 市级
        this.dialog.formData.tellerlvl = '3'
        this.dialog.config.tellerlvl.options = tellerData.filter(item => item.value !== '1' && item.value !== '2')
        datas[0].children = children.filter(item => item.id === '33' || item.id === '44')
      } else if (!num) { // admin
        this.dialog.formData.tellerlvl = '1'
      } else {
        this.dialog.config.tellerlvl.options = tellerData.filter(item => item.value === '4')
        datas[0].children = children.filter(item => item.id === '44')
        this.dialog.formData.tellerlvl = '4'
      }
    },
    /**
     * btn - 筛选角色号数据*/
    ...
    */
    ...
    dialogAddSubmit() {
      ...
      const { role_no } = this.dialog.formData
      if (role_no === '1' || role_no === '2') {
        commonMsgInfo('角色号选择有误!', this)
        return
      }
