      ...
      <template slot="tableColumn">
        ...
        <el-table-column label="本月开户投诉（单位：起）" >
          <el-table-column label="地方性银行" align="center">
            <el-table-column prop="thismonth_complain_people" label="人民银行受理">
              <div slot-scope="{ row }">
                <span>{{ row.thismonth_complain_people }}</span>
              </div>
            </el-table-column>
            <el-table-column prop="thismonth_complain_bank" label="银行受理">
              <div slot-scope="{ row }">
                <span>{{ row.thismonth_complain_bank }}</span>
              </div>
            </el-table-column>
          </el-table-column>
        </el-table-column>
        <el-table-column label="本年1月至本月开户投诉（单位：起）">
          <el-table-column label="地方性银行" align="center">
            <el-table-column prop="tothismonth_complain_people" label="人民银行受理">
              <div slot-scope="{ row }">
                <span>{{ row.tothismonth_complain_people }}</span>
              </div>
            </el-table-column>
            <el-table-column prop="tothismonth_complain_bank" label="银行受理">
              <div slot-scope="{ row }">
                <span>{{ row.tothismonth_complain_bank }}</span>
              </div>
            </el-table-column>
          </el-table-column>
        </el-table-column>
        <el-table-column label="本年1月至本月开户服务满意度调查" align="center">
          <el-table-column prop="recover_num_of_ques" label="收回有效调查问卷份数">
            <div slot-scope="{ row }">
              <span>{{ row.recover_num_of_ques }}</span>
            </div>
          </el-table-column>
          <el-table-column label="问卷结果" align="center">
            <el-table-column prop="very_satisfied_num" label="其中：非常满意的份数">
              <div slot-scope="{ row }">
                <span>{{ row.very_satisfied_num }}</span>
              </div>
            </el-table-column>
            <el-table-column prop="quite_satisfied_num" label="其中：比较满意的份数">
              <div slot-scope="{ row }">
                <span>{{ row.quite_satisfied_num }}</span>
              </div>
            </el-table-column>
            <el-table-column prop="satisfied_num" label="其中：满意的份数">
              <div slot-scope="{ row }">
                <span>{{ row.satisfied_num }}</span>
              </div>
            </el-table-column>
            <el-table-column prop="dissatisfied_num" label="其中：不满意的份数">
              <div slot-scope="{ row }">
                <span>{{ row.dissatisfied_num }}</span>
              </div>
            </el-table-column>
            <el-table-column
              prop="very_dissatisfied_num"
              label="其中：非常不满意的份数"
            >
              <div slot-scope="{ row }">
                <span>{{ row.very_dissatisfied_num }}</span>
              </div>
            </el-table-column>
          </el-table-column>
        </el-table-column>
        <el-table-column label="本年1月至本月累计减免费用（单位万元）" align="center">
          <el-table-column prop="open_manage_fee" label="开户管理费">
            <div slot-scope="{ row }">
              <span>{{ row.open_manage_fee }}</span>
            </div>
          </el-table-column>
          <el-table-column prop="year_fee" label="年费">
            <div slot-scope="{ row }">
              <span>{{ row.year_fee }}</span>
            </div>
          </el-table-column>
          <el-table-column prop="online_open_fee" label="网银开通费">
            <div slot-scope="{ row }">
              <span>{{ row.online_open_fee }}</span>
            </div>
          </el-table-column>
          <el-table-column prop="other_manage_fee" label="其他与账户相关管理费">
            <div slot-scope="{ row }">
              <span>{{ row.other_manage_fee }}</span>
            </div>
          </el-table-column>
          <el-table-column prop="total_fee" label="合计">
            <div slot-scope="{ row }">
...
import { EntSatisfied } from '@/api'
const { query, add, del } = EntSatisfied
export default {
  name: 'TableList',
  ...
  methods: {
    ...
    handleExport() {
      ...
      exportXlsx(tHeader, data, '企业开户满意度监测表')
    }
  }
}
</script>
